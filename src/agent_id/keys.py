...
def public_key_from_did_key(did: str) -> bytes:
    ...
    if decoded[0] != 0xED or decoded[1] != 0x01:
        ...
        raise ValueError(
            f"unsupported key type in did:key (expected Ed25519 multicodec 0xed01, got {got})"
        )
...
def public_key_from_multibase(value: str) -> bytes | None:
    """Decode a Multikey publicKeyMultibase. Returns the raw 32-byte key for Ed25519, else None."""
...
def base58btc_decode(value: str) -> bytes:
    if not value.startswith("z"):
        raise ValueError(f"multibase string must start with 'z' (base58btc), got {value[:1]!r}")
    return bytes(base58.b58decode(value[1:]))
